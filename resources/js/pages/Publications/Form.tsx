...
export default function Form({ initialData, url, method = 'post' }: Props) {
  ...
  return (
    <form onSubmit={submit} className="space-y-4 max-w-md">
        <div>
            <label className="block mb-1">Institucion</label>
            <select
            value={data.institution_id}
            onChange={e => setData('institution_id', e.target.value)}
            className="border p-2 w-full rounded"
            >
            <option value="1" className='text-black' >Federacion</option>
            <option value="2" className='text-black'>Sindicato</option>
            </select>
            {errors.institution_id && <div className="text-red-500">{errors.institution_id}</div>}
        </div>
        <div>
            <label className="block mb-1">Tipo Publicacion</label>
            <select
            value={data.type_publication_id}
            onChange={e => setData('type_publication_id', e.target.value)}
            className="border p-2 w-full rounded"
            >
            <option value="1" className='text-black' >Condolencia</option>
            <option value="2" className='text-black'>Cumpleaños</option>
            </select>
            {errors.type_publication_id && <div className="text-red-500">{errors.type_publication_id}</div>}
        </div>
